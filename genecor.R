...
panel.cor <- function(x, y, sigfigs=2, prefix="", sig_threshold=0.5,  cex.cor, ...) {
	...
	text(0.5, 0.5, txt, cex = cex.cor*1.0, col=c("gray60, black")[(abs(r)>sig_threshold)+1])
}
...
genes <- c("Irx3$", "Sirt1$", "Ptpn1$") # and others, there's no upper limit other than practical
...
pairs(cross$pheno[,4:length(cross$pheno)], upper.panel=panel.cor, diag.panel=panel.hist)
...
# Perform a QTL scan
scan1 <- scanone(cross_gp, pheno.col = cat(pheno, genes), method="hk", addcovar=sex)
...
perm1 <- scanone(cross, pheno.col = (4+length(pheno)), addcovar = sex, method="hk", n.perm=100, perm.Xsp=TRUE)
